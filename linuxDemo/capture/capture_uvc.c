...
#if (USE_MALLOC)
// 保存v4l2内部所用的缓冲区指针数组
static dmabuf_buffer_t **save_used_malloc_buffers = NULL;
static struct v4l2_buffer buf_from_malloc;
/**
 * @brief 初始化UVC摄像头
 * @param width 捕获宽
 * @param height 捕获高
 ...
 * @return 成功返回0 失败返回-1
 */
int capture_uvc_init_malloc(uint32_t width, uint32_t height,
    ...
    // 检查保存缓冲区的指针数组参数
    save_used_malloc_buffers = malloc(sizeof(dmabuf_buffer_t *) * frames);
    if (!save_used_malloc_buffers) {
    ...
    for (int i = 0; i < frames; i++) {
        ...
        // 申请成功，保存缓冲区指针
        save_used_malloc_buffers[i] = buffer;
        buf_alloc_cnt++;
    }
    ...
    memset(&buf_from_malloc, 0, sizeof(buf_from_malloc));
    for (int i = 0; i < frames; i++) {
        buf_from_malloc.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        buf_from_malloc.memory = V4L2_MEMORY_USERPTR;
        buf_from_malloc.index = i;
        buf_from_malloc.m.userptr =
            (unsigned long)save_used_malloc_buffers[i]->data;
        buf_from_malloc.length = save_used_malloc_buffers[i]->size;

        // 将缓冲区加入队列
        if (ioctl(v4l2_fd, VIDIOC_QBUF, &buf_from_malloc) < 0) {
    ...
    if (save_used_malloc_buffers) {
        for (int i = 0; i < buf_alloc_cnt; i++) {
            dmabuf_buffer_force_free(alloc_buf_from_pool,
                                     save_used_malloc_buffers[i]);
        }
        free(save_used_malloc_buffers);
        save_used_malloc_buffers = NULL;
    }
    // 继续处理
error_close:
    if (v4l2_fd >= 0) {
 ...
 */
dmabuf_buffer_t *capture_uvc_captureImg_malloc(dmabuf_buffer_t *next_buffer) {
    // 检查设备状态
    ...
    dmabuf_buffer_t *filled_buffer =
        save_used_malloc_buffers[filled_v4l2_buf.index]; // 利用索引获取
    /*******简要说明filled_buffer*******/
    ...
    // 4. 更新数组指针为新缓冲区
    save_used_malloc_buffers[filled_v4l2_buf.index] = next_buffer;
 ...
 */
void capture_uvc_clean_malloc(dmabuf_pool_t *alloc_buf_from_pool) {
    if (v4l2_fd < 0 || frames_local == 0) {
        fprintf(stderr, "摄像头未初始化\n");
        return;
    }
    ...
    for (int i = 0; i < frames_local; i++) {
        buffer = save_used_malloc_buffers[i];
        if (!buffer)
            continue;
        ...
        save_used_malloc_buffers[i] = NULL;
    }
    // 3. 关闭设备
    close(v4l2_fd);
    v4l2_fd = -1;
...
#elif (USE_DMABUF)
// 保存v4l2内部所用的缓冲区指针数组
static dmabuf_buffer_t **save_used_dmabuf_buffers = NULL;
static struct v4l2_buffer buf_from_dmabuf;
// dmabuf优化专用
/**
 * @brief 初始化UVC摄像头
 * @param width 捕获宽
